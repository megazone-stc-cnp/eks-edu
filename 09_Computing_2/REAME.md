...
## 2-3. NodeGroup에서 Fargate로 전환 작업 ( HMMA )
### 2-3-1. 작업 절차
1. EFS용 PV/PVC 생성
...
aws eks create-fargate-profile \
  --cluster-name HAE-PRD-HMMA-EKS1-UE1 \
  --pod-execution-role-arn arn:aws:iam::281413867167:role/HAE-PRD-SF-IAMROL-FARGATE-EKS-UE1 \
  --fargate-profile-name ops-kube-system \
  --selectors '[{"namespace": "kube-system"}]' \
  --subnets '["subnet-072702d9da74c0f4b", "subnet-0fbc55ce2b9af91cf"]' \
  --profile hmma
``` 
5. CoreDNS 재시작
```shell
...
```shell
kubectl exec -it datastudiofbcc-be-depl-76647f6ddf-82xqk -- sh
nslookup autoway.hyundai.net
-> 10.6.180.43
```
...
aws eks create-fargate-profile \
  --cluster-name HAE-PRD-HMMA-EKS1-UE1 \
  --pod-execution-role-arn arn:aws:iam::281413867167:role/HAE-PRD-SF-IAMROL-FARGATE-EKS-UE1 \
  --fargate-profile-name app-ns-hva1 \
  --selectors '[{"namespace": "hva1"}]' \
  --subnets '["subnet-072702d9da74c0f4b", "subnet-0fbc55ce2b9af91cf"]' \
  --profile hmma
```
9. EKS Addon 삭제
