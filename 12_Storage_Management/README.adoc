...
|ReadWriteOncePod
|하나의 Pod만 읽기/쓰기 가능
|동일 노드의 다른 Pod도 접근 불가 - 강력한 독점 접근 보장
|===

== Reclaim Policy

PVC가 삭제되었을 때 PV와 그에 연결된 실제 스토리지 자원을 어떻게 처리할 지 정의
...
|Retain
|PVC 삭제 시 PV와 실제 스토리지는 그대로 유지
|관리자가 직접 수동으로 PV를 정리해야 함 - 데이터 유실 방지 가능

|Delete
|PVC 삭제 시 PV와 실제 스토리지(데이터)도 자동 삭제
|자동 정리됨 - 데이터 복구 불가(영구 삭제됨)

|Recycle(Deprecated)
|PVC 삭제 시 PV 내용을 rm -rf 로 초기화한 후 재사용 가능
|보안상 안정하지 않음
|===

== Volume Binding Mode

PVC와 PV가 언제, 어떤 조건에서 바인딩 되고 실제 스토리지가 프로비저닝 되는지를 정의

.Volume Binding Mode 종류
[%autowidth,cols="1s,a,a,a"]
|===
|이름 |동작 방식 |특징 |사례

|Immediate
|PVC 생성되는 즉시 PV와 바인딩
|PVC가 생성되면 바로 PV와 연결 시도
|EFS

|WaitForFirstConsumer
|PVC를 사용하는 첫 번째 Pod가 스케줄링될 때 PV와 바인딩
|Pod 스케줄링 시점에 바인딩됨
|EBS
|===
