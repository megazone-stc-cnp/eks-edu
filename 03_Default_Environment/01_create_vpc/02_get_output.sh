...
echo "#!/bin/bash" >> ${VPC_ENV_FILE_PATH}
echo "export VPC_ID=$(cat result.json | jq -r '.[] | select(.OutputKey=="VpcId") | .OutputValue')" >> ${VPC_ENV_FILE_PATH}
echo "export AWS_AZ1=$(cat result.json | jq -r '.[] | select(.OutputKey=="PrivateSubnet01AZ") | .OutputValue')" >> ${VPC_ENV_FILE_PATH}
echo "export AWS_AZ2=$(cat result.json | jq -r '.[] | select(.OutputKey=="PrivateSubnet02AZ") | .OutputValue')" >> ${VPC_ENV_FILE_PATH}
echo "export AWS_PRIVATE_SUBNET1=$(cat result.json | jq -r '.[] | select(.OutputKey=="PrivateSubnet01") | .OutputValue')" >> ${VPC_ENV_FILE_PATH}
echo "export AWS_PRIVATE_SUBNET2=$(cat result.json | jq -r '.[] | select(.OutputKey=="PrivateSubnet02") | .OutputValue')" >> ${VPC_ENV_FILE_PATH}
